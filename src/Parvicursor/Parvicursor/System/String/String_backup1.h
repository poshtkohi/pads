	...
	public: String *Trim()
		{
			...
			if(startIndex >= 0 && endIndex > 0)
			{
				...
				char  *newBuffer = new char[len + 1]; //delete
				for(int i = startIndex + 1 , j = 0; i < endIndex ; i++, j++)
					newBuffer[j] =  this->bytes[i];
				newBuffer[len] = 0;
				String *temp = new String((const char *)newBuffer);
				delete newBuffer;
				return temp;
			}
			...
			{
				char* newBuffer = new char[endIndex + 1]; //delete
			...
			{
				String *temp = new String((const char *)this->bytes);
				return temp;
			}
		}
		//----------------------------------------------------
		public: String *Substring(int startIndex, int count)
		{
			...
			buffer[count] = 0;
			String *temp = new String((const char *)buffer);
			delete buffer;
			return temp;
		}
		//----------------------------------------------------
		...
		*/
		public: String *ToUpper()
		{
			if(bytes == null)
				return null;
			String *buffer = Copy(this);
			for(int i = 0 ; i < this->size ; i++)
				if(buffer->get_Value(i) >= 'a' && buffer->get_Value(i) <= 'z')
					buffer->GetBaseStream()[i] -= 32;
			return buffer;
		}
		//----------------------------------------------------
		...
		*/
		...
		{
			if(str.GetBaseStream() == null)
				return ;
			else
			{
			   char *b = new char[this->get_Length() + str.get_Length() + 1];
			   int index = str.get_Length() + (this->get_Length() - str.get_Length());
		...
		*/
		...
		{
			if(StrB.GetBaseStream() == null)
				return Copy(&StrA);
			if(StrA.GetBaseStream() == null)
				return Copy(&StrB);
		...
		*/
		...
		{
			if(StrA.GetBaseStream() == null && StrB.GetBaseStream() == null)
				return 0;
			if(null ==  StrA.GetBaseStream() && null == StrB.GetBaseStream())
				return -1;
			if(null == StrA.GetBaseStream() && null == StrB.GetBaseStream())
				return -1;
			else
	    ...
	    */
		...
		public: String *Replace(String &oldValue, String &newValue)
		{
			...
			while(true)
			{
				...
				{
					p1 = this->IndexOf(oldValue, 0, p1);
					if(p1 >= 0)
					{
						
						al.Add((Object *)p1);
						p1 = oldValue.get_Length() + p1;
					}
					else 
						break;
				}
			...
			else
			{
				...
				if((int)al[al.Count - 1] + oldValue.get_Length() < this->get_Length())
					str = str + this->Substring((int)al[al.get_Count() - 1] + oldValue.get_Length());
				al.Clear();
				//al = null;
				return str;
			}
