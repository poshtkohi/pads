...
namespace Parvicursor
{
	...
	public: String Trim()
		...
		private: String *Substring_Pointer(int startIndex, int count)
		{
			if(bytes == null)
				return this;
			//add here exception for startIndex and count
		...
		*/
		...
		public: friend String operator+(String &StrA, const char *StrB)
		{
			String b(StrB);
			return StrA + b;
		}
		public: friend String operator+(const char *StrA, String &StrB)
		{
			String a(StrA);
			return a + StrB;
		}
		...
		/*public: String Replace(String &oldValue, String &newValue)
		{
			...
			else
			{
				...
				for(int i = 0 ; i < al.get_Count() ; i++)
				{
...
#else
						String *s = Substring_Pointer(0, (int)al[i]);
						str = *s + newValue;
						delete s;
#endif
					}
					...
					{
#if defined WIN
						str = str + this->Substring((int)al[i - 1] + oldValue.get_Length(), (int)al[i] - (int)al[i - 1] - oldValue.get_Length());
#else
						String *s = Substring_Pointer((int)al[i - 1] + oldValue.get_Length(), (int)al[i] - (int)al[i - 1] - oldValue.get_Length());
						str = str + *s;
						delete s;
#endif
						str = str + newValue;
...
#else
					String *s = Substring_Pointer((int)al[al.get_Count() - 1] + oldValue.get_Length(), this->get_Length());
					str = str + *s;
					delete s;
#endif
				}
