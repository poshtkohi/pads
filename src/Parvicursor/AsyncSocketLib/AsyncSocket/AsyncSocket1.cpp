        ...
        namespace Sockets
        {
                    ...
                    if(isAccepted)
                    {
                        if(asyncResult !=  null)
                        {
                            ...
                            if(manager != null && threadInstance != null && !isClosedByRuntimeManager && manager != null) // Removes from reader/writer threads
                            {
                                ...
                                if(found)
                                    AsyncSocketRuntimeManager::SignalReaderWriterThreadToUnblockEpollWait(threadInstance);
			...
			IAsyncResult *AsyncSocket::BeginAccept(AsyncCallback callback, Object *state)
			{
			    ...
			    if(!serverMode)
			    {
                    ...
                    serverModeRuntime = new AsyncSocketRuntimeManager(internalSocket, Environment::get_ProcessorCount() * 2, true); //
			...
			IAsyncResult *AsyncSocket::BeginConnect(IPEndPoint &remoteEP, AsyncCallback callback, Object *state)
			{
                ...
                fdState->asyncResult = this->asyncResult;
            ...
            bool AsyncSocket::TryMigrateToCore(Int32 DestCoreID)
            {
                ...
                manager->SignalReaderWriterThreadToUnblockEpollWait(targetThreadInstance);
                manager->SignalReaderWriterThreadToUnblockEpollWait(destThreadInstance);
